...
#include "unstructured_fluent_mesh.h"
namespace SPH
{
    //=================================================================================================//
    void readFluentMeshFile_3d::getDataFromFluentMeshFile3d()
    {
        ...
        /*point_coordinates_3D_.resize(number_of_points + 1);
        for (std::size_t yy = 0; yy != number_of_points + 1; ++yy)
        {
            point_coordinates_3D_[yy].resize(dimension);
            for (std::vector<std::vector<double>>::size_type point = 0; point != point_coordinates_3D_[yy].size(); ++point)
            {
                point_coordinates_3D_[yy][point] = 0;
            }
        }*/
        ...
        while (getline(mesh_file, text_line))
        {
            ...
            std::ofstream outfile1("Points_coordinate.txt");
            if (outfile1.is_open())
            {
                outfile1 << std::fixed << std::setprecision(15);
                // Iterate through the vector and write each element to the file
                for (const auto& points : point_coordinates_3D_)
                {
                    for (const double xyz : points)
                    {
                        outfile1 << xyz << " ";
                    }
    ...
    void readFluentMeshFile_3d::getFluentElementCenterCoordinates()
    {
        ...
        for (std::vector<std::vector<long unsigned int>>::size_type element = 1; element != elements_nodes_connection_.size(); ++element)
        {
            ...
            for (std::vector<long unsigned int>::size_type node = 0; node != elements_nodes_connection_[element].size(); ++node)
            {
                if (point_coordinates_3D_[elements_nodes_connection_[element][node]][0] == 0.25000000000000000 && point_coordinates_3D_[elements_nodes_connection_[element][node]][1] == 0.0000000000000000 && point_coordinates_3D_[elements_nodes_connection_[element][node]][2] == 0.10000000149011611)
                {
                    Real cds = 1; 
                }
            ...
            if (node4_coordinate[0] == 0.25000000000000000 && node4_coordinate[1] == 0.0000000000000000 && node4_coordinate[2] == 0.10000000149011611)
            {
                Real jj = 199;
            }
...
void readFluentMeshFile_3d::getMinimumDistanceBetweenNodesFluent()
{
...
//=============================================================================================//
  ...
  //=================================================================================================//
  //=================================================================================================//
FluentParticleGeneratorInFVM::FluentParticleGeneratorInFVM(SPHBody &sph_body, const StdLargeVec<Vecd> &positions, const StdLargeVec<Real> &elements_volumes)
    : ParticleGenerator(sph_body), elements_center_coordinates_(positions), elements_volumes_(elements_volumes) {}
//=================================================================================================//
void FluentParticleGeneratorInFVM::initializeGeometricVariables()
{
...
void BodyStatesRecordingInFluentMeshToVtu::writeWithFileName(const std::string &sequence)
{
