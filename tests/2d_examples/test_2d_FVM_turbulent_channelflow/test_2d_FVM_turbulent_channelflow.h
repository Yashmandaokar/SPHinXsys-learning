...
//----------------------------------------------------------------------
std::string mesh_file_path = "./input/Height2MChannel.msh";
//
...
public:
    ...
    void applyNonSlipWallBoundary(size_t ghost_index, size_t index_i) override
    {
        ...
        K_[ghost_index] = 0.0;
        Eps_[ghost_index] = 0.0;
        mu_t_[ghost_index] = mu_t_[index_i];
        
        if (index_i == 2622)
        {
            Vecd velghwall = vel_[ghost_index];
            Real kwall = K_[ghost_index];
            Real Epswall = Eps_[ghost_index];
            Real x = 1.0;
        }  
    }
